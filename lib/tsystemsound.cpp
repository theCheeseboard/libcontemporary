...
tSystemSound::tSystemSound(QObject* parent) :
    QObject(parent) {
}
...
QSettings::Format getDesktopFormat() {
    if (desktopFormat == QSettings::InvalidFormat) {
        desktopFormat = QSettings::registerFormat(
            "desktop", [](QIODevice& device, QSettings::SettingsMap& map) -> bool {
                QString group;
                while (!device.atEnd()) {
...
tSystemSound* tSystemSound::play(QString soundName, qreal volume) {
    ...
    if (soundPath != "") {
        ...
        output->setVolume(qRound(QAudio::convertVolume(volume, QAudio::LogarithmicVolumeScale, QAudio::LinearVolumeScale) * 100));
        QMediaPlayer* player = new QMediaPlayer();
        player->setAudioOutput(output);
        player->setSource(QUrl::fromLocalFile(soundPath));
        connect(player, &QMediaPlayer::playbackStateChanged, engine, [=](QMediaPlayer::PlaybackState state) {
            if (state == QMediaPlayer::StoppedState) {
                // We're done here
                emit engine->done();
